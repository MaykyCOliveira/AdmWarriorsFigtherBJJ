// Carrega os alunos do localStorage ou inicia com um array vazio
let alunos = JSON.parse(localStorage.getItem("alunos")) || [];

// Garante que cada aluno possua um identificador único e a propriedade 'frequencia'
alunos.forEach((aluno, index) => {
  if (!aluno.id) {
    aluno.id = Date.now() + index; // ou use outra estratégia de geração de id
  }
  if (!aluno.frequencia) {
    aluno.frequencia = {};
  }
...
// Salva os alunos (incluindo os dados de frequência) no localStorage
...
// Gera a tabela de frequência ao carregar a página
document.addEventListener("DOMContentLoaded", gerarTabelaFrequencia);
